...
```{r, echo=FALSE, message=FALSE}
  ...
  source("src/time_series.R")
  source("src/raster.R")
  
  # tidy up the data and preprocess
  t2m_ras <- rast("data/era5_temperature2m.nc")
  t2m_ras <- t2m_ras["t2m_expver=1_*"]
  t2m_ras <- t2m_ras[[1:501]]
  names(t2m_ras) <- time(t2m_ras)
...
Additionally you can add the worl map without filling, but with the borders alone. Here we load the packages again we know from former classes:
...
* Fix the problem through storing `t2m_ras_new[[1]]` to a variable and change the `extend` accordingly
